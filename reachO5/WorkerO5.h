...
//		vector<NODETYPE> inBL, ouBL;
		//-----new agent version---------
		koala::my_openadd_hashset inAgent, ouAgent;
		vector<NODETYPE> inrAgent, ourAgent;
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_forward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {
					...
					for (NODETYPE idx:out_ghosts[msg.first]) {
						...
						if (v.iVisit.intersection(&mir[msg.second].ouAgent))continue;//prune by batch partial label
						
						if (v.level <= msglevel) {
							if (v.level < msglevel)
								ou_inc_agent.insert({msg.second, v.level - batch_begin_lvl});
							continue;//pruning
						}
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_backward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {
					...
					for (NODETYPE idx:in_ghosts[msg.first]) {
						...
						if (v.oVisit.intersection(&mir[msg.second].inAgent))continue;//prune by batch partial label
						
						
						if (v.level <= msglevel) {
							if (v.level < msglevel)
								in_inc_agent.insert({msg.second, v.level - batch_begin_lvl});
							continue;//pruning
						}
...
//						}
		...
		void share_inc_Batch_reach_info() {
			vector<pair<NODETYPE, NODETYPE>> _in_inc_agent(in_inc_agent.begin(), in_inc_agent.end()), _ou_inc_agent(
					ou_inc_agent.begin(), ou_inc_agent.end());
			in_inc_agent.clear(), ou_inc_agent.clear();
			
			
			vector<vector<pair<NODETYPE, NODETYPE >>> _in_inc_agent_info = Ibin_all_share_all(_in_inc_agent);
			vector<vector<pair<NODETYPE, NODETYPE >>> _ou_inc_agent_info = Ibin_all_share_all(_ou_inc_agent);

//			if (MASTER_RANK == me) {
//				int cnt0 = 0;
//				for (const auto &e:_in_inc_agent_info) cnt0 += e.size();
//				for (const auto &e:_ou_inc_agent_info) cnt0 += e.size();
//				log_general("inc cnt0: %d", cnt0)
//
//				if (cnt0 < 100 && cnt0 > 0) {
//					cout << "in ";
//					for (const auto &e:_in_inc_agent_info)
//						for (const auto &p:e)
//							cout << "(" << p.first << "," << p.second << ") ";
//					cout << endl;
//					cout << "ou ";
//					for (const auto &e:_ou_inc_agent_info)
//						for (const auto &p:e)
//							cout << "(" << p.first << "," << p.second << ") ";
//					cout << endl;
//				}
//			}
			for (auto &a:_in_inc_agent_info) {
				for (auto b:a) {
					mir[b.second].inrAgent.emplace_back(b.first);
					
					function<void(NODETYPE, const vector<NODETYPE> &)> dfs_insert =
							[&dfs_insert](NODETYPE v, const vector<NODETYPE> &inc_agents) {
								vector<NODETYPE> _inc_agents;
								for (auto e:inc_agents) {
									if (mir[v].inAgent.insert(e))_inc_agents.push_back(e);
								}
								if (_inc_agents.size() > 0) {
									for (auto e:mir[v].inrAgent) {
										dfs_insert(e, _inc_agents);
									}
								}
							};
					
					vector<NODETYPE> inc_agents;
					inc_agents.emplace_back(b.second);
					for (auto e:mir[b.second].inAgent.release_data()) {
						if (e != nullele)inc_agents.emplace_back(e);
					}
					dfs_insert(b.first, inc_agents);
				}
			}
			for (auto &a:_ou_inc_agent_info) {
				for (auto b:a) {
					mir[b.second].ourAgent.emplace_back(b.first);
					
					function<void(NODETYPE, const vector<NODETYPE> &)> dfs_insert =
							[&dfs_insert](NODETYPE v, const vector<NODETYPE> &inc_agents) {
								vector<NODETYPE> _inc_agents;
								for (auto e:inc_agents) {
									if (mir[v].ouAgent.insert(e))_inc_agents.push_back(e);
								}
								if (_inc_agents.size() > 0) {
									for (auto e:mir[v].ourAgent) {
										dfs_insert(e, _inc_agents);
									}
								}
							};
					...
					vector<NODETYPE> inc_agents;
					inc_agents.emplace_back(b.second);
					for (auto e:mir[b.second].ouAgent.release_data()) {
						if (e != nullele)inc_agents.emplace_back(e);
					}
					dfs_insert(b.first, inc_agents);
				}
...
//				mir[i].ouBL.clear(), mir[i].ouBL.shrink_to_fit();
				mir[i].inAgent.clear();
				mir[i].ouAgent.clear();
				mir[i].inrAgent.clear();
				mir[i].ourAgent.clear();
			}
		...
		void postbatch() {
			for (NODETYPE idx:post_batch_idx) {
				...
				for (NODETYPE a:oVec) {
					if (a != 4294967295U) {
						if (!v.oVisit.intersection(&mir[a].inAgent))tmpl.emplace_back(a + batch_begin_lvl);
					}
				}
				v.oVisit.clear();
				...
				for (NODETYPE a:iVec) {
					if (a != 4294967295U) {
						if (!v.iVisit.intersection(&mir[a].ouAgent))tmpl.emplace_back(a + batch_begin_lvl);
					}
				}
				v.iVisit.clear();
				sort(tmpl.begin(), tmpl.end());
			...
			//record the size of agents
			if (MASTER_RANK == me) {
				for (const auto &m:mir) {
					pstat.ouAgentCnt += m.ouAgent.size();
					pstat.inAgentCnt += m.inAgent.size();
				}
			}
...
//                cout << "Input: " << setting::inputFileDirectory << endl;
			...
			ResetTimer(COMMUNICATION_TIMER);
...
//
//				record("inAgentCnt:%u ouAgentCnt:%u avgAgentCnt:%f",
//				       pstat.inAgentCnt,
//				       pstat.ouAgentCnt,
...
//            }
	...
	private:
		...
		unordered_set<pair<NODETYPE, NODETYPE>> in_inc_agent, ou_inc_agent;
