...
//            post_batch_idx.shrink_to_fit();
  ...
  vector<centralized_vertex> load_txt_graph() {
...
//    }
    ...
    for (string &path:assignedSplits) {
      ifstream ifs(path);
      if (!ifs) {
        log_general("open file %s failed.", path.c_str())
        exit(-1);
      }
      NODE_TYPE vid, deg, neb;
      //skip comments
      auto pos = ifs.tellg();
      string line;
...
//        oss << "vid:" << vtxs[i].id + 1 << " level:" << vtxs[i].level + 1;
//        oss << " ou:";
//        for (int j = 0; j < vtxs[i].oneb.size(); ++j) {
//          oss << vtxs[i].oneb[j] + 1 << " ";
//        }
//        oss << " in:";
//        for (int j = 0; j < vtxs[i].ineb.size(); ++j) {
//          oss << vtxs[i].ineb[j] + 1 << " ";
//        }
//        oss << endl;
//      }
