#ifndef WORKER_Hagentana
#define WORKER_Hagentana
...
//                    iidx_msg[peer].shrink_to_fit();
        ...
        void postbatch() {
            for (int i = 0; i < batch_end_lvl - batch_begin_lvl; i++) {
                agentcnt += mir[i].inBL.size();
                agentcnt += mir[i].ouBL.size();
            }
...
//            PrintTimer("Total Pre_Batch Time", PRE_BATCH_TIMER);
            if (me == MASTER_RANK) {
                ...
                record("agentcnt is: %lu", agentcnt);
            }
            worker_barrier();
