...
namespace O4b {
    ...
    inline void set_bit(unsigned long &bitmap, unsigned int pos) {
#define mask 63u
        bitmap |= 1ul << (pos & mask);
#undef mask
    }
    ...
    class Vertex {
    public:
        Vertex() : ilbitmap(0), olbitmap(0), iVbitmap(0), oVbitmap(0) {}

        Vertex(centralized_vertex &v) : ilbitmap(0), olbitmap(0), iVbitmap(0), oVbitmap(0) {
            id = v.id;
            level = v.level;
        ...
        unsigned long iVbitmap, oVbitmap;
        unsigned long ilbitmap, olbitmap;
...
//===========================================Batch Relevant Code====================
    ...
    struct mirror_vertex {
        mirror_vertex() : iBLbitmap(0), oBLbitmap(0), ibitmap(0), obitmap(0) {}

        mirror_vertex(NODETYPE id, vector<NODETYPE> &in, vector<NODETYPE> &ou, unsigned long ib, unsigned long ob) :
                id(id), in(in), out(ou), iBLbitmap(0), oBLbitmap(0), ibitmap(ib), obitmap(ob) {}

        void
        set(NODETYPE id1, const vector<NODETYPE> &in1, const vector<NODETYPE> &out1, unsigned long ib,
            unsigned long ob) { id = id1, in = in1, out = out1, ibitmap = ib, obitmap = ob; }

        NODETYPE id;
        vector<NODETYPE> in;
        vector<NODETYPE> out;
        vector<NODETYPE> inBL, ouBL;
        unsigned long iBLbitmap, oBLbitmap;
        unsigned long ibitmap, obitmap;
    };
            ...
            for (int peer = 0; peer < np; ++peer) {
                if (peer != me) {
                    ...
                    for (size_t i = 0; i < oidx_msg[peer].size(); i++) {
                        ibss[peer] << oidx_msg[peer][i] << mir[oidx_msg[peer][i]].out << mir[oidx_msg[peer][i]].obitmap;
...
//                    oidx_msg[peer].shrink_to_fit();
                    ...
                    for (size_t i = 0; i < iidx_msg[peer].size(); i++) {
                        ibss[peer] << iidx_msg[peer][i] << mir[iidx_msg[peer][i]].in << mir[iidx_msg[peer][i]].ibitmap;
            ...
            for (int peer = 0; peer < np; ++peer) {
                if (peer != me) {
                    ...
                    for (size_t i = 0; i < size; ++i) {
                        obss[peer] >> id;
                        mir[id].id = id;
                        obss[peer] >> mir[id].out >> mir[id].obitmap;
                    }
                    obss[peer] >> size;
                    for (size_t i = 0; i < size; ++i) {
                        obss[peer] >> id;
                        mir[id].id = id;
                        obss[peer] >> mir[id].in >> mir[id].ibitmap;
                    }
                }
            }
        }
            ...
            for (vector<pair<NODETYPE, NODETYPE>> &a:_forward_gmsg) {
                for (pair<NODETYPE, NODETYPE> msg:a) {
                    ...
                    for (NODETYPE idx:out_ghosts[msg.first]) {
                        ...
                        if ((v.ilbitmap & mir[msg.second].obitmap) && set_intersection(v.ilb, mir[msg.second].out))
                            continue;//prune with pre batch partial label

                        if ((v.iVbitmap & mir[msg.second].iBLbitmap) && v.iVisit.intersection(mir[msg.second].ouBL))
                            continue;//prune by batch partial label
...
//                        {//debug
//                            if (msglevel == 1 && v.id == 0) {
//                                printf("0's olbitmap:%lo 2's ilbitmap:%lo\n", v.olbitmap, mir[msg.second].ibitmap);
//                            }
//                        }
                        if ((v.olbitmap & mir[msg.second].ibitmap) && set_intersection(v.olb, mir[msg.second].in))
                            continue;//prune with pre batch partial label
                        if ((v.oVbitmap & mir[msg.second].iBLbitmap) && v.oVisit.intersection(mir[msg.second].inBL))
                            continue;//prune by batch partial label
        ...
        void share_inc_Batch_reach_info() {
            ...
            for (auto &a:_in_inc_reach_info) {
                for (auto b:a) {
                    ...
                    set_bit(mir[b.first].iBLbitmap, b.second);
                }
            }
            for (auto &a:_ou_inc_reach_info) {
                for (auto b:a) {
                    mir[b.first].ouBL.emplace_back(b.second);
                    set_bit(mir[b.first].oBLbitmap, b.second);
                }
            }
        }
        ...
        void prebatch() {
            ...
            for (int i = 0; i < batch_end_lvl - batch_begin_lvl; i++) {
                ...
                mir[i].oBLbitmap = 0u, mir[i].iBLbitmap = 0u;
            }
            ...
            if (share_before_batch) {
                ...
                for (map<NODETYPE, NODETYPE>::iterator it = lvl_v_idx.lower_bound(batch_begin_lvl);
                    ...
                    Vertex &v = vertexes[it->second];
                    to_send.emplace_back(inBatchLevel(v.level), v.ilb, v.olb, v.ilbitmap, v.olbitmap);
                }
                ...
                for (map<NODETYPE, NODETYPE>::iterator it = lvl_v_idx.lower_bound(batch_begin_lvl);
                     it != lvl_v_idx.end() && it->first < batch_end_lvl; ++it) {
                    Vertex &v = vertexes[it->second];
                    mir[inBatchLevel(v.level)].set(inBatchLevel(v.level), v.ilb, v.olb, v.ilbitmap, v.olbitmap);
                }
                for (int i = 0; i < np; i++) {
                    if (i != me) {
        ...
        void postbatch() {
            for (NODETYPE idx:post_batch_idx) {
                ...
                for (NODETYPE a:oVec) {
                    if (a != 4294967295U) {
                        if (!(v.oVbitmap & mir[a].iBLbitmap) || !v.oVisit.intersection(mir[a].inBL)) {
                            tmpl.emplace_back(a + batch_begin_lvl);
                            set_bit(v.olbitmap, tmpl.back());
                        }
                    }
                }
                v.oVisit.clear();
                ...
                for (NODETYPE a:iVec) {
                    if (a != 4294967295U) {
                        if (!(v.iVbitmap & mir[a].oBLbitmap) || !v.iVisit.intersection(mir[a].ouBL)) {
                            tmpl.emplace_back(a + batch_begin_lvl);
                            set_bit(v.ilbitmap, tmpl.back());
                        }
                    }
                }
                v.iVisit.clear();
