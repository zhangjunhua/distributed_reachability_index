...
#define WORKER_Ho3a
/**
 * This version of code is based on O3a.
 * The purpose of creating this code version is that for submission of icde2021, we try to
 * figure out a proper batch size schema that suits all kinds of datasets, especially suits
 * web graphs, like hostlinkage, webspamuk, etc.
 */
#include <vector>
#include <fstream>
...
namespace O3a {
	...
	public:
		...
		koala::my_openadd_hashset invst, ivvst, onvst, ovvst;
...
//                    iidx_msg[peer].shrink_to_fit();
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_del_forward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {//id,level
					for (NODETYPE idx:out_ghosts[msg.first]) {
						...
						if (v.ivvst.find(msg.second))continue;
						if (set_intersection(v.olb, mir[msg.second].in))continue;
						v.ivvst.insert(msg.second);
						status_update(idx);
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_del_backward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {
					for (NODETYPE idx:in_ghosts[msg.first]) {
						Vertex &v = vertexes[idx];
						if (v.level < batch_begin_lvl)continue;
						if (v.ovvst.find(msg.second))continue;//has been deleted
						if (set_intersection(v.olb, mir[msg.second].in))continue;
						v.ovvst.insert(msg.second);
						status_update(idx);
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_add_forward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {
					...
					for (NODETYPE idx:out_ghosts[msg.first]) {
						Vertex &v = vertexes[idx];
						if (v.level < batch_begin_lvl)continue;
						if (v.ivvst.find(msg.second) || v.invst.find(msg.second))continue;  //has been deleted or added
						if (set_intersection(v.ilb, mir[msg.second].out))continue;
						if (msglevel < v.level) {
							v.invst.insert(msg.second);
							status_update(idx);
			...
			for (vector<pair<NODETYPE, NODETYPE>> &a:_add_backward_gmsg) {
				for (pair<NODETYPE, NODETYPE> msg:a) {
					...
					for (NODETYPE idx:in_ghosts[msg.first]) {
						Vertex &v = vertexes[idx];
						if (v.level < batch_begin_lvl)continue;
						if (v.ovvst.find(msg.second) || v.onvst.find(msg.second))continue;//has been added or deleted
						if (set_intersection(v.olb, mir[msg.second].in))continue;
						if (msglevel < v.level) {
							v.onvst.insert(msg.second);
							status_update(idx);
		...
		void postbatch() {
			for (NODETYPE idx:post_batch_idx) {
				Vertex &v = vertexes[idx];
				vector<NODETYPE> &oVec = v.onvst.release_data();
				vector<NODETYPE> tmpl;
				for (NODETYPE a:oVec) {
					if (a != 4294967295U && !v.ovvst.find(a)) {
				...
				vector<NODETYPE> iVec = v.invst.release_data();
				tmpl.clear();
				for (NODETYPE a:iVec) {
					if (a != 4294967295U && !v.ivvst.find(a)) {
						tmpl.emplace_back(a + batch_begin_lvl);
					}
				}
				v.invst.clear();
...
//            }
	...
	private:
		...
		hash_map<NODETYPE, vector<NODETYPE>>
				out_ghosts,
				in_ghosts; //vid->nbs's idx
