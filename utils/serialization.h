...
ibinstream &operator<<(ibinstream &m, const pair<T, T1> &v) {
    m << v.first << v.second;
    return m;
}
...
public:
    obinstream() : buf(NULL), size(0), index(0) {}
...
obinstream &operator>>(obinstream &m, unsigned int &i) {
    i = *(unsigned int *) m.raw_bytes(sizeof(int));
    return m;
}
...
template<typename T, typename T1>
obinstream &operator>>(obinstream &m, pair<T, T1> &v) {
    m >> v.first >> v.second;
    return m;
}
