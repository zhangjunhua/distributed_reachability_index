...
string namenode_HostName = "master";

void setHDFSHost(string hostname) {
    namenode_HostName = hostname;
}

hdfsFS getHdfsFS() {
    hdfsFS fs = hdfsConnect(namenode_HostName.c_str(), 9000);
    if (!fs) {
...
struct LineReader {
    ...
    void appendLine() {
        if (bufPos == bufSize)
            return;
        //find '\n' from bufPos to bufend, return the first
        char *pch = (char *) memchr(buf + bufPos, '\n', bufSize - bufPos);
        if (pch == NULL) {//'\n' not found
            lineAppend(buf + bufPos, bufSize - bufPos);//append tail to the line
            bufPos = bufSize;
            if (!fileEnd)
                fill();
            else
...
{ //locality is not considered for simplicity
    vector<vector<string> > *assignmentPointer = new vector<vector<string> >(np);
    ...
    tOffset *assigned = new tOffset[np];
    for (int i = 0; i < np; i++)
        assigned[i] = 0;
    //sort files by size
    vector<sizedFName> sizedfile;
