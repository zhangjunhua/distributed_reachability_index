...
public:
    ...
    void load_vertex(TeraItem<T> *v) { //called by load_graph
        add_vertex(v);
    }
    ...
    void load_graph(const char *inpath) {
        ...
        hdfsCloseFile(fs, in);
        hdfsDisconnect(fs);
        //cout<<"Worker "<<me<<": \""<<inpath<<"\" loaded"<<endl;//DEBUG !!!!!!!!!!
    }
    //=======================================================
    ...
    void key_sampling(vector<T> &splits) //key<=splits[i], to machine i
    {
        ...
        if (me != MASTER_RANK) {
            ...
            //compute splits
            int gap = samples.size() / np;
            if (gap == 0)
                splits.swap(samples);
            else {
                int residual = samples.size() % np;
                int pos = 0;
                for (int i = 0; i < np - 1; i++) //last sample may not be the largest item
                {
                    int step = gap;
                    if (residual > 0) {
                        step++;
    ...
    void prefixOrder() {
        ...
        if (me != MASTER_RANK) {
            ...
            //collect sizes
            vector<int> sizebuf(np);
            sizebuf[me] = vertexes.size();
    ...
    void run(const WorkerParams &params) {
        //check path + init
        if (me == MASTER_RANK) {
            if (dirCheck(params.input_path.c_str(), params.output_path.c_str(), me == MASTER_RANK,
                         params.force_write) == -1)
                exit(-1);
        }
        init_timers();
        ...
        if (me == MASTER_RANK) {
            arrangement = params.native_dispatcher ? dispatchLocality(params.input_path.c_str()) : dispatchRan(
